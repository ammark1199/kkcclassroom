...
export default function HelpSupport() {
  ...
  const [showGetInTouchEmail, setShowGetInTouchEmail] = useState(false);
    ...
    <div className="min-h-screen bg-[#F1EFE7] text-[#000] font-sans px-6 py-12 flex flex-col items-center space-y-12 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl w-full">
        ...
        >
          ...
          {showSupportEmail && (
            <div className="mt-4 text-sm bg-[#F1EFE7] border border-[#FF637A]/20 rounded-lg px-4 py-3">
              <p className="text-gray-800">
                Email us at <span className="font-semibold text-[#FF637A]">KKChelp@KKC.com</span>
              </p>
            </div>
          )}
        </div>
      ...
      <div className="text-center space-y-4 pt-16">
        ...
        <button
          onClick={() => setShowGetInTouchEmail(!showGetInTouchEmail)}
          className="bg-[#FF637A] hover:bg-pink-600 text-white px-8 py-3 rounded-xl shadow-md text-lg font-medium transition"
        >
          Get in Touch
        </button>
        {showGetInTouchEmail && (
          <div className="mt-4 text-sm bg-white border border-gray-300 rounded-xl px-6 py-4 shadow">
            <p className="text-gray-700">
              Email us at <span className="font-semibold text-[#FF637A]">KKCinfo@KKC.com</span>
            </p>
          </div>
        )}
      </div>
