...
export default function Dashboard({ selectedTab }) {
  ...
  return (
    <div className="p-10 min-h-screen bg-[#F1EFE7]">
      <div className="text-4xl font-bold mb-8 text-black">Welcome Back! ({selectedTab})</div>
      ...
      <div className="grid grid-cols-3 gap-8">
          ...
          {courses.map((course, index) => (
            <div key={index} className="mb-4">
              <div className="flex justify-between items-center mb-1">
                ...
                <button className="bg-[#FF637A] text-white px-3 py-1 rounded-md hover:bg-pink-600">
                  Resume
                </button>
              </div>
              <div className="w-full bg-[#F1EFE7] h-2 rounded">
                <div
                  className="bg-[#FF637A] h-2 rounded"
                  style={{ width: `${course.progress}%` }}
                ></div>
              </div>
            </div>
          ))}
          <button className="mt-3 bg-black text-white px-4 py-2 rounded-md hover:bg-[#333333]">
            + Add Course
          </button>
        </div>
        ...
        {/* Resources */}
        <div className="bg-white rounded-lg p-5 shadow-md">
          <h2 className="text-2xl font-bold mb-4 text-black">Resources</h2>
          {resources.map((resource, index) => (
            <a
              key={index}
              href={resource.link}
              className="block text-[#FF637A] hover:underline mb-2"
            >
              {resource.name}
            </a>
          ))}
        ...
        <div className="bg-white rounded-lg p-5 shadow-md">
          ...
          <ul className="list-disc pl-5 text-black space-y-2">
            {questions.map((question, index) => (
              <li key={index}>
                <a href="#" className="text-[#FF637A] hover:underline">
                  {question}
                </a>
              </li>
            ))}
      ...
      <div className="grid grid-cols-2 gap-8 mt-8">
        {/* Recently Accessed */}
        <div className="bg-white rounded-lg p-5 shadow-md">
          <h2 className="text-2xl font-bold mb-4 text-black">Recently Accessed</h2>
        ...
        {/* More Questions */}
        <div className="bg-white rounded-lg p-5 shadow-md">
          <h2 className="text-2xl font-bold mb-4 text-black">More Questions?</h2>
          <ul className="list-disc pl-5 text-black">
